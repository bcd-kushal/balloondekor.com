...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed top-3 z-[100] flex max-h-screen w-full flex-col-reverse p-4 sm:bottom-4 sm:right-1 sm:top-auto sm:flex-col md:max-w-fit sm:min-w-[300px]",
      className
    )}
    {...props}
  />
...
const toastVariants = cva(
  "group text-[16px] my-2 pointer-events-auto hover:shadow-2xl relative flex w-full items-center justify-between space-x-2 overflow-hidden rounded-lg p-5 pr-6 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        ...
        success: "bg-green-400 text-green-950/90",
        warning:
          "bg-amber-500 text-neutral-950/90"
      }
    },
    defaultVariants: {
      variant: "default"
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Title
    ref={ref}
    className={cn(
      "text-[13px] sm:text-[18px] font-semibold [&+div]:text-[13px] sm:[&+div]:text-[16px]",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Description
    ref={ref}
    className={cn(
      "text-[14px] sm:text-[16px] opacity-90",
      className
    )}
    {...props}
  />
